...
# Declare shielding/vessel inputs before run_openmc so the BOL leakage
...
update_params({
    ...
    # Three activation zones are used through the concrete depth on both the
    # side wall and base: 0-5 cm, 5-25 cm, and 25-200 cm.
    ...
    # Higher statistics are used only for the shutdown-photon calculation;
    # neutron transfer and activation remain at the faster settings above.
    'Shielding Photon Particles': 50000,
    'Shielding Photon Batches': 10,
    # If the first photon tally is zero, rerun only photon transport at 4x
...
params.update({
    ...
    'Primary Loop Count': 1,  # number of primary coolant loops in the plant
    'Primary Loop per loop load fraction': 1.0,  # single loop handles the full thermal load
